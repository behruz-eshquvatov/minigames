var F=Object.defineProperty;var B=(m,e,a)=>e in m?F(m,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):m[e]=a;var n=(m,e,a)=>B(m,typeof e!="symbol"?e+"":e,a);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))s(t);new MutationObserver(t=>{for(const r of t)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&s(i)}).observe(document,{childList:!0,subtree:!0});function a(t){const r={};return t.integrity&&(r.integrity=t.integrity),t.referrerPolicy&&(r.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?r.credentials="include":t.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(t){if(t.ep)return;t.ep=!0;const r=a(t);fetch(t.href,r)}})();class R{constructor(){n(this,"element");n(this,"backdrop");n(this,"dialogCard");n(this,"currentMode","login");n(this,"isOpen",!1);n(this,"handleKeyDown",e=>{e.key==="Escape"&&this.close()});this.backdrop=document.createElement("div"),this.backdrop.className="auth-backdrop",this.dialogCard=document.createElement("div"),this.dialogCard.className="auth-dialog",this.backdrop.append(this.dialogCard),this.element=this.backdrop,this.renderContent(),this.attachEvents()}getElement(){return this.element}open(e="login"){this.currentMode=e,this.renderContent(),document.body.append(this.element),this.isOpen=!0,requestAnimationFrame(()=>{this.backdrop.classList.add("auth-backdrop--active"),this.dialogCard.classList.add("auth-dialog--active")}),document.addEventListener("keydown",this.handleKeyDown)}close(){this.isOpen&&(this.backdrop.classList.remove("auth-backdrop--active"),this.dialogCard.classList.remove("auth-dialog--active"),setTimeout(()=>{this.element.parentNode&&this.element.remove(),this.isOpen=!1},250),document.removeEventListener("keydown",this.handleKeyDown))}attachEvents(){this.backdrop.addEventListener("click",e=>{e.target===this.backdrop&&this.close()})}switchMode(e){if(this.currentMode===e)return;this.currentMode=e;const a=this.dialogCard.querySelector(".auth-dialog__form-container");a?(a.classList.add("auth-dialog__form-container--transitioning"),setTimeout(()=>{this.renderContent();const s=this.dialogCard.querySelector(".auth-dialog__form-container");s&&s.classList.remove("auth-dialog__form-container--transitioning")},150)):this.renderContent()}renderContent(){const e=this.currentMode==="login";this.dialogCard.innerHTML=`
    ...
    `;const a=this.dialogCard.querySelectorAll(".auth-dialog__switcher-btn");for(const i of a)i.addEventListener("click",()=>{const o=i.dataset.mode;this.switchMode(o)});const s=this.dialogCard.querySelectorAll(".auth-switch-link");for(const i of s)i.addEventListener("click",o=>{o.preventDefault();const c=i.dataset.mode;this.switchMode(c)});const t=this.dialogCard.querySelectorAll(".auth-field__toggle-pwd");for(const i of t)i.addEventListener("click",()=>{var c;const o=(c=i.parentElement)==null?void 0:c.querySelector("input");if(o){const d=o.type==="password";o.type=d?"text":"password",i.innerHTML=d?this.getEyeOffIconSvg():this.getEyeIconSvg()}});const r=this.dialogCard.querySelector("form");r&&r.addEventListener("submit",i=>{i.preventDefault()})}renderLoginForm(){return`
      ...
      <div class="game-details-dialog" role="dialog" aria-modal="true" aria-labelledby="game-details-title">
        <div class="game-details-dialog__content">
          ...
          <div class="game-details-dialog__hero">
            <img src="${$}" alt="${a.name}" class="game-details-dialog__hero-image" />
          ...
          <div class="game-details-dialog__body">
            ...
            <section class="game-details-dialog__info-section">
              <div class="game-details-dialog__header-meta">
                <h2 id="game-details-title" class="game-details-dialog__title">${a.name}</h2>
                <div class="game-details-dialog__stats">
                  ...
                  <span class="game-details-dialog__likes">
                    <img src="${L}" alt="Likes" />
                    ${(a.likesCount/1e3).toFixed(1)}K
                  </span>
                </div>
              </div>

              <div class="game-details-dialog__badges">
                <span class="game-details-dialog__badge">Genre: ${a.specs.genre}</span>
                <span class="game-details-dialog__badge">Players: ${a.specs.players}</span>
                <span class="game-details-dialog__badge">Duration: ${a.specs.duration}</span>
                <span class="game-details-dialog__badge">Price: ${a.specs.price}</span>
              </div>
    ...
    `,e}setupListeners(){this.backdropElement.addEventListener("click",t=>{t.target===this.backdropElement&&this.close()});const e=this.backdropElement.querySelector(".game-details-dialog__hero-close-btn");e&&e.addEventListener("click",()=>{this.close()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&this.backdropElement.classList.contains("game-details-backdrop--active")&&this.close()}),this.favoriteBtnElement=this.backdropElement.querySelector(".game-details-dialog__favorite-btn")??void 0,this.favoriteBtnElement&&this.favoriteBtnElement.addEventListener("click",()=>{var r,i,o;this.isFavorite=!this.isFavorite,(r=this.favoriteBtnElement)==null||r.classList.toggle("game-details-dialog__favorite-btn--active",this.isFavorite);const t=(i=this.favoriteBtnElement)==null?void 0:i.querySelector(".fav-btn-text");t&&(t.textContent=this.isFavorite?"Remove from Favorites":"Add to Favorites"),(o=this.favoriteBtnElement)==null||o.setAttribute("aria-label",this.isFavorite?"Remove from Favorites":"Add to Favorites")}),this.textareaElement=this.backdropElement.querySelector(".game-details-dialog__textarea")??void 0;const a=this.backdropElement.querySelector(".game-details-dialog__submit-comment-btn");this.textareaElement&&this.textareaElement.addEventListener("input",()=>{if(!this.textareaElement)return;this.textareaElement.style.height="auto";const t=Math.min(this.textareaElement.scrollHeight,88);this.textareaElement.style.height=`${t}px`,a&&(a.disabled=this.textareaElement.value.trim().length===0)}),a&&a.addEventListener("click",()=>{if(!this.textareaElement||this.textareaElement.value.trim().length===0)return;const t=this.textareaElement.value.trim(),r=this.backdropElement.querySelector(".game-details-dialog__comments-list");if(r){const i=document.createElement("article");i.className="game-details-dialog__comment-item";const o=`comment-${Date.now()}`;this.commentLikes.set(o,{count:0,active:!1}),i.innerHTML=`
            <div class="game-details-dialog__comment-header">
              <span class="game-details-dialog__comment-author">You</span>
              <time class="game-details-dialog__comment-date">Just now</time>
            </div>
            <p class="game-details-dialog__comment-text">${t}</p>
            <div class="game-details-dialog__comment-footer">
              <button
                type="button"
                class="game-details-dialog__comment-like-btn"
    ...
    `,e}}class Y{constructor(e={}){n(this,"element");n(this,"isMenuOpen",!1);n(this,"burgerButton");n(this,"mobileMenuOverlay");n(this,"callbacks");n(this,"activePage");this.callbacks=e,this.activePage=e.activePage||"home",this.element=this.createHeaderElement(),this.setupEventListeners()}getElement(){return this.element}setActivePage(e){this.activePage=e;const a=this.element.querySelectorAll(".site-header__nav-link");for(const t of a){const r=t.dataset.page;t.classList.toggle("site-header__nav-link--active",r===e)}const s=this.mobileMenuOverlay.querySelectorAll(".mobile-menu__nav-link");for(const t of s){const r=t.dataset.page;t.classList.toggle("mobile-menu__nav-link--active",r===e)}}createHeaderElement(){const e=document.createElement("header");e.className="site-header";const a=document.createElement("div");a.className="site-header__container";const s=document.createElement("a");s.href="#",s.className="site-header__logo",s.innerHTML=`
    ...
    `,s.addEventListener("click",h=>{var g,f;h.preventDefault(),this.setActivePage("home"),(f=(g=this.callbacks).onNavigate)==null||f.call(g,"home")});const t=document.createElement("nav");t.className="site-header__nav",t.innerHTML=`
    ...
    `;const r=t.querySelector('[data-page="home"]');r&&r.addEventListener("click",h=>{var g,f;h.preventDefault(),this.setActivePage("home"),(f=(g=this.callbacks).onNavigate)==null||f.call(g,"home")});const i=t.querySelector('[data-page="library"]');i&&i.addEventListener("click",h=>{var g,f;h.preventDefault(),this.setActivePage("library"),(f=(g=this.callbacks).onNavigate)==null||f.call(g,"library")});const o=document.createElement("button");o.className="btn btn--outline site-header__login-btn",o.textContent="Log In",o.addEventListener("click",()=>{var h,g;this.closeMobileMenu(),(g=(h=this.callbacks).onLoginClick)==null||g.call(h)});const c=document.createElement("button");c.className="btn btn--primary site-header__signup-btn",c.textContent="Sign Up",c.addEventListener("click",()=>{var h,g;this.closeMobileMenu(),(g=(h=this.callbacks).onSignUpClick)==null||g.call(h)});const d=document.createElement("div");d.className="site-header__nav-wrapper",d.append(t,o),this.burgerButton=document.createElement("button"),this.burgerButton.className="site-header__burger-btn",this.burgerButton.setAttribute("aria-label","Toggle menu"),this.burgerButton.innerHTML=`
      <span class="site-header__burger-icon">
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
      </span>
    `;const p=document.createElement("div");p.className="site-header__right-controls",p.append(d,c,this.burgerButton),a.append(s,p),e.append(a),this.mobileMenuOverlay=document.createElement("div"),this.mobileMenuOverlay.className="mobile-menu",this.mobileMenuOverlay.innerHTML=`
    ...
    `,e}getVisibleGames(){const e=this.games.length;return["narrow","normal","wide","normal","narrow"].map((s,t)=>{const r=(this.centerIndex+(t-2)+e*10)%e;return{game:this.games[r],widthType:s}})}renderTrack(){const e=this.element.querySelector(".carousel-section__track");if(!e)return;const a=this.getVisibleGames();e.innerHTML=a.map(t=>`
    ...
    `).join("");const s=e.querySelectorAll(".game-card");for(const t of s){const r=t.dataset.slug,i=this.games.find(o=>o.slug===r);i&&(t.addEventListener("click",()=>{var o,c;(c=(o=this.callbacks).onGameClick)==null||c.call(o,i)}),t.addEventListener("keydown",o=>{var c,d;(o.key==="Enter"||o.key===" ")&&(o.preventDefault(),(d=(c=this.callbacks).onGameClick)==null||d.call(c,i))}))}this.checkCardWidthThresholds()}checkCardWidthThresholds(){const e=this.element.querySelector(".carousel-section__track");if(!e)return;const a=e.querySelectorAll(".game-card");for(const s of a){const t=s.querySelector(".game-card__overlay");if(t){const r=s.getBoundingClientRect().width;r>0&&r<288?t.style.display="none":r>=288&&(t.style.display="")}}}next(){this.centerIndex=(this.centerIndex+1)%this.games.length,this.renderTrack()}prev(){this.centerIndex=(this.centerIndex-1+this.games.length)%this.games.length,this.renderTrack()}startAutoplay(){this.stopAutoplay(),this.autoplayTimer=setInterval(()=>{this.next()},4e3)}stopAutoplay(){this.autoplayTimer!==void 0&&(clearInterval(this.autoplayTimer),this.autoplayTimer=void 0)}resetAutoplay(){this.startAutoplay()}setupControls(){const e=this.element.querySelector(".carousel-section__control-btn--prev"),a=this.element.querySelector(".carousel-section__control-btn--next");e&&e.addEventListener("click",()=>{this.prev(),this.resetAutoplay()}),a&&a.addEventListener("click",()=>{this.next(),this.resetAutoplay()});const s=this.element.querySelector(".carousel-section__track-wrapper");s&&(s.addEventListener("pointerdown",t=>{this.startX=t.clientX,this.isSwiping=!0,this.stopAutoplay()}),s.addEventListener("pointerup",t=>{if(!this.isSwiping)return;this.isSwiping=!1;const r=t.clientX-this.startX;Math.abs(r)>40?(r<0?this.next():this.prev(),this.resetAutoplay()):this.startAutoplay()}),s.addEventListener("pointercancel",()=>{this.isSwiping=!1,this.startAutoplay()}),s.addEventListener("mouseenter",()=>{this.stopAutoplay()}),s.addEventListener("mouseleave",()=>{this.isSwiping=!1,this.startAutoplay()})),window.addEventListener("resize",()=>{this.checkCardWidthThresholds()})}destroy(){this.stopAutoplay()}}const le=""+new URL("random-image-DsQZrXDY.png",import.meta.url).href,ce="data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M8%202V10M4.66667%205.33333L8%202L11.3333%205.33333M14%2010V12.6667C14%2013.0203%2013.8595%2013.3594%2013.6095%2013.6095C13.3594%2013.8595%2013.0203%2014%2012.6667%2014H3.33333C2.97971%2014%202.64057%2013.8595%202.39052%2013.6095C2.14048%2013.3594%202%2013.0203%202%2012.6667V10'%20stroke='%23242145'%20stroke-width='2'%20stroke-linecap='round'/%3e%3c/svg%3e";class de{constructor(){n(this,"element");this.element=this.createGameDevElement()}getElement(){return this.element}createGameDevElement(){const e=document.createElement("section");return e.className="game-dev-section",e.innerHTML=`
    ...
    `,e}}class ge{constructor(){n(this,"element");this.element=this.createHeroElement()}getElement(){return this.element}createHeroElement(){const e=document.createElement("section");return e.className="hero-section",e.innerHTML=`
      ...
      <div class="leaderboard-section__container">
        ...
        <div class="leaderboard-table-card">
          <table class="leaderboard-table">
            ...
            <tbody>
              ${a.map(s=>`
                <tr class="leaderboard-row leaderboard-row--rank-${s.rank}">
                  <td class="col-rank">
                    <span class="rank-badge rank-badge--${s.rank}">#${s.rank}</span>
                  </td>
                  <td class="col-player">
                    <div class="player-info">
                      <span class="player-avatar" style="background-color: ${he[s.rank]||"#E0EEF6"}">
                        ${pe(s.playerName)}
                      </span>
                      <span class="player-name">${s.playerName}</span>
                    </div>
                  </td>
                  <td class="col-games">${s.gamesPlayed}</td>
                  <td class="col-score">
                    <span class="score-desktop">${E(s.totalScore,!1)}</span>
                    <span class="score-mobile">${E(s.totalScore,!0)}</span>
                  </td>
                  <td class="col-streak">
                    <span class="streak-full">🔥 ${s.streakDays} days</span>
                    <span class="streak-short">🔥 ${s.streakDays}d</span>
                  </td>
                  <td class="col-favorite">
                    <span class="favorite-badge">${s.favoriteGameName}</span>
                  </td>
                </tr>
              `).join("")}
            </tbody>
            ...
            <ul class="library-filter-section__sort-dropdown" role="listbox">
              ${s.map(t=>`
                <li class="library-filter-section__sort-item ${t===this.activeSort?"library-filter-section__sort-item--active":""}" role="option" data-sort="${t}">
                  <span class="library-filter-section__sort-check">${t===this.activeSort?"✔":""}</span>
                  <span class="library-filter-section__sort-text">${t}</span>
                </li>
              `).join("")}
            </ul>
          </div>
    ...
    `,e}getFilteredAndSortedGames(){let e=[...this.games];switch(this.activeCategory!=="all"&&(e=e.filter(a=>a.category.toLowerCase()===this.activeCategory.toLowerCase())),this.activeSort){case"Rating ↑":{e.sort((a,s)=>a.rating-s.rating);break}case"Rating ↓":{e.sort((a,s)=>s.rating-a.rating);break}case"Name A→Z":{e.sort((a,s)=>a.name.localeCompare(s.name));break}case"Name Z→A":{e.sort((a,s)=>s.name.localeCompare(a.name));break}}return e}renderCards(){const e=this.element.querySelector(".library-grid-section__grid");if(!e)return;const a=this.getFilteredAndSortedGames();this.totalPages=Math.max(1,Math.ceil(a.length/this.itemsPerPage));const s=(this.currentPage-1)*this.itemsPerPage,t=a.slice(s,s+this.itemsPerPage);e.innerHTML=t.map(i=>{const o=this.getGameImageUrl(i.slug),c=this.formatLikes(i.likesCount);return`
      ...
      `}).join("");const r=e.querySelectorAll(".library-card__details-btn");for(const i of r)i.addEventListener("click",()=>{var d,p;const o=i.dataset.slug,c=this.games.find(l=>l.slug===o);c&&((p=(d=this.callbacks).onGameDetailsClick)==null||p.call(d,c))})}setupInteractivity(){const e=this.element.querySelector(".library-pagination__arrow-btn--prev"),a=this.element.querySelector(".library-pagination__arrow-btn--next"),s=this.element.querySelectorAll(".library-pagination__page-btn"),t=l=>{this.currentPage=Math.min(l,this.totalPages),e&&(e.disabled=this.currentPage<=1),a&&(a.disabled=this.currentPage>=this.totalPages);for(const u of s){const _=Number(u.dataset.page);u.classList.toggle("library-pagination__page-btn--active",_===this.currentPage),u.style.display=_>this.totalPages?"none":""}this.renderCards()},r=this.element.querySelectorAll(".library-filter-section__chip");for(const l of r)l.addEventListener("click",()=>{const u=l.dataset.category;if(u){this.activeCategory=u;for(const _ of r){const b=_.dataset.category===u;_.classList.toggle("library-filter-section__chip--active",b),_.setAttribute("aria-selected",b.toString())}t(1)}});const i=this.element.querySelector(".library-filter-section__chips-wrapper");if(i){let l=!1,u=0,_=0;i.addEventListener("mousedown",b=>{l=!0,u=b.pageX-i.offsetLeft,_=i.scrollLeft}),i.addEventListener("mouseleave",()=>{l=!1}),i.addEventListener("mouseup",()=>{l=!1}),i.addEventListener("mousemove",b=>{if(!l)return;b.preventDefault();const y=(b.pageX-i.offsetLeft-u)*1.5;i.scrollLeft=_-y})}const o=this.element.querySelector(".library-filter-section__sort-btn"),c=this.element.querySelector(".library-filter-section__sort-dropdown"),d=this.element.querySelectorAll(".library-filter-section__sort-item"),p=this.element.querySelector(".sort-current-label");if(o&&c){o.addEventListener("click",l=>{l.stopPropagation(),this.isSortOpen=!this.isSortOpen,o.classList.toggle("library-filter-section__sort-btn--open",this.isSortOpen),c.classList.toggle("library-filter-section__sort-dropdown--open",this.isSortOpen),o.setAttribute("aria-expanded",this.isSortOpen.toString())});for(const l of d)l.addEventListener("click",()=>{const u=l.dataset.sort;if(u){this.activeSort=u,p&&(p.textContent=u);for(const _ of d){const b=_.dataset.sort===u;_.classList.toggle("library-filter-section__sort-item--active",b);const v=_.querySelector(".library-filter-section__sort-check");v&&(v.textContent=b?"✔":"")}this.isSortOpen=!1,o.classList.remove("library-filter-section__sort-btn--open"),c.classList.remove("library-filter-section__sort-dropdown--open"),o.setAttribute("aria-expanded","false"),t(1)}});document.addEventListener("click",l=>{this.element.contains(l.target)||(this.isSortOpen=!1,o.classList.remove("library-filter-section__sort-btn--open"),c.classList.remove("library-filter-section__sort-dropdown--open"),o.setAttribute("aria-expanded","false"))})}for(const l of s)l.addEventListener("click",()=>{const u=Number(l.dataset.page);u&&t(u)});e&&e.addEventListener("click",()=>{this.currentPage>1&&t(this.currentPage-1)}),a&&a.addEventListener("click",()=>{this.currentPage<this.totalPages&&t(this.currentPage+1)})}}class Qe{constructor(e){n(this,"routes",[]);n(this,"rootElement");n(this,"currentPath","");this.rootElement=e,window.addEventListener("popstate",()=>{this.handleRoute()}),window.addEventListener("hashchange",()=>{this.handleRoute()})}addRoute(e){this.routes.push(e)}navigate(e){window.location.hash=e.startsWith("#")?e:`#${e}`}getCurrentRoute(){return window.location.hash.replace(/^#\/?/,"")==="library"||window.location.pathname.replaceAll(/^\/|\/$/g,"").endsWith("library")?"library":"home"}handleRoute(){const a=this.getCurrentRoute()==="library"?"/library":"/";if(this.currentPath===a&&this.rootElement.children.length>0)return;this.currentPath=a;const s=this.routes.find(t=>t.path===a);if(this.rootElement.innerHTML="",s)this.rootElement.append(s.render());else{const t=this.routes.find(r=>r.path==="/");t&&this.rootElement.append(t.render())}}}function Je(){var c;const m=document.querySelector("#app");if(!m)return;const e=new Qe(m),a=new R,s=new G,t=new Y({activePage:"home",onLoginClick:()=>{a.open("login")},onSignUpClick:()=>{a.open("register")},onNavigate:d=>{e.navigate(d==="library"?"/library":"/")}}),r=new X,o=r.getElement().querySelectorAll(".site-footer__link");for(const d of o){const p=(c=d.textContent)==null?void 0:c.trim().toLowerCase();p==="home"?d.addEventListener("click",l=>{l.preventDefault(),t.setActivePage("home"),e.navigate("/")}):p==="library"&&d.addEventListener("click",l=>{l.preventDefault(),t.setActivePage("library"),e.navigate("/library")})}e.addRoute({path:"/",render:()=>{t.setActivePage("home");const d=document.createElement("div");d.className="app-container app-container--home",d.append(t.getElement());const p=document.createElement("main");p.className="main-content";const l=new ge,u=new ne({onGameClick:()=>{s.open()}}),_=new _e,b=new de;return p.append(l.getElement(),u.getElement(),_.getElement(),b.getElement()),d.append(p,r.getElement()),d}}),e.addRoute({path:"/library",render:()=>{t.setActivePage("library");const d=document.createElement("div");d.className="app-container app-container--library",d.append(t.getElement());const p=document.createElement("main");p.className="main-content";const l=new Ye({onGameDetailsClick:()=>{s.open()}});return p.append(l.getElement()),d.append(p,r.getElement()),d}}),e.handleRoute()}document.addEventListener("DOMContentLoaded",()=>{Je()});
