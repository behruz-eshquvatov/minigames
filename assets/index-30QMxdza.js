var F=Object.defineProperty;var B=(m,e,s)=>e in m?F(m,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):m[e]=s;var r=(m,e,s)=>B(m,typeof e!="symbol"?e+"":e,s);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))a(t);new MutationObserver(t=>{for(const i of t)if(i.type==="childList")for(const o of i.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&a(o)}).observe(document,{childList:!0,subtree:!0});function s(t){const i={};return t.integrity&&(i.integrity=t.integrity),t.referrerPolicy&&(i.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?i.credentials="include":t.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(t){if(t.ep)return;t.ep=!0;const i=s(t);fetch(t.href,i)}})();class R{constructor(){r(this,"element");r(this,"backdrop");r(this,"dialogCard");r(this,"currentMode","login");r(this,"isOpen",!1);r(this,"handleKeyDown",e=>{e.key==="Escape"&&this.close()});this.backdrop=document.createElement("div"),this.backdrop.className="auth-backdrop",this.dialogCard=document.createElement("div"),this.dialogCard.className="auth-dialog",this.backdrop.append(this.dialogCard),this.element=this.backdrop,this.renderContent(),this.attachEvents()}getElement(){return this.element}open(e="login"){this.currentMode=e,this.renderContent(),document.body.append(this.element),this.isOpen=!0,requestAnimationFrame(()=>{this.backdrop.classList.add("auth-backdrop--active"),this.dialogCard.classList.add("auth-dialog--active")}),document.addEventListener("keydown",this.handleKeyDown)}close(){this.isOpen&&(this.backdrop.classList.remove("auth-backdrop--active"),this.dialogCard.classList.remove("auth-dialog--active"),setTimeout(()=>{this.element.parentNode&&this.element.remove(),this.isOpen=!1},250),document.removeEventListener("keydown",this.handleKeyDown))}attachEvents(){this.backdrop.addEventListener("click",e=>{e.target===this.backdrop&&this.close()})}switchMode(e){if(this.currentMode===e)return;this.currentMode=e;const s=this.dialogCard.querySelector(".auth-dialog__form-container");s?(s.classList.add("auth-dialog__form-container--transitioning"),setTimeout(()=>{this.renderContent();const a=this.dialogCard.querySelector(".auth-dialog__form-container");a&&a.classList.remove("auth-dialog__form-container--transitioning")},150)):this.renderContent()}renderContent(){const e=this.currentMode==="login";this.dialogCard.innerHTML=`
    ...
    `;const s=this.dialogCard.querySelectorAll(".auth-dialog__switcher-btn");for(const o of s)o.addEventListener("click",()=>{const n=o.dataset.mode;this.switchMode(n)});const a=this.dialogCard.querySelectorAll(".auth-switch-link");for(const o of a)o.addEventListener("click",n=>{n.preventDefault();const g=o.dataset.mode;this.switchMode(g)});const t=this.dialogCard.querySelectorAll(".auth-field__toggle-pwd");for(const o of t)o.addEventListener("click",()=>{var g;const n=(g=o.parentElement)==null?void 0:g.querySelector("input");if(n){const l=n.type==="password";n.type=l?"text":"password",o.innerHTML=l?this.getEyeOffIconSvg():this.getEyeIconSvg()}});const i=this.dialogCard.querySelector("form");i&&i.addEventListener("submit",o=>{o.preventDefault()})}renderLoginForm(){return`
      ...
      <div class="game-details-dialog" role="dialog" aria-modal="true" aria-labelledby="game-details-title">
        <div class="game-details-dialog__content">
          ...
          <div class="game-details-dialog__hero">
            <img src="${H}" alt="${s.name}" class="game-details-dialog__hero-image" />
          ...
          <div class="game-details-dialog__body">
            ...
            <section class="game-details-dialog__info-section">
              ...
              <div class="game-details-dialog__badges">
                <span class="game-details-dialog__badge">Genre: ${s.specs.genre}</span>
                <span class="game-details-dialog__badge">Players: ${s.specs.players}</span>
                <span class="game-details-dialog__badge">Duration: ${s.specs.duration}</span>
                <span class="game-details-dialog__badge">Price: ${s.specs.price}</span>
              </div>
            ...
            <section class="game-details-dialog__records-section">
              ...
              <table class="game-details-dialog__records-table">
                ...
                <tbody>
                  ${s.topRecords.map(t=>`
                    <tr>
                      <td class="record-rank">${t.position}</td>
                      <td class="record-player">${t.playerName}</td>
                      <td class="record-score">${t.score.toLocaleString()}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
                ...
                ${a.map(t=>`
                  <article class="game-details-dialog__comment-item">
                    <div class="game-details-dialog__comment-header">
                      <span class="game-details-dialog__comment-author">${t.authorName}</span>
                      <time class="game-details-dialog__comment-date">${new Date(t.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</time>
                    </div>
                    <p class="game-details-dialog__comment-text">${t.text}</p>
                    <div class="game-details-dialog__comment-footer">
                      <button
                        type="button"
                        class="game-details-dialog__comment-like-btn"
                        data-comment-id="${t.commentId}"
                        aria-label="Like comment by ${t.authorName}"
                      >
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                          <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path>
                        </svg>
                        <span class="like-count">${t.likesCount}</span>
                      </button>
                    </div>
                  </article>
                `).join("")}
      ...
      <div class="site-footer__container">
        ...
        <div class="site-footer__bottom">
          ...
          <a href="https://rs.school/courses/short-track" target="_blank" rel="noopener noreferrer" class="site-footer__credit-link">
            <img src="${Z}" alt="RS School Logo" class="site-footer__rs-logo" />
            <span>RS School</span>
          </a>
    ...
    `,e}}class Q{constructor(e={}){r(this,"element");r(this,"isMenuOpen",!1);r(this,"burgerButton");r(this,"mobileMenuOverlay");r(this,"callbacks");r(this,"activePage");this.callbacks=e,this.activePage=e.activePage||"home",this.element=this.createHeaderElement(),this.setupEventListeners()}getElement(){return this.element}setActivePage(e){this.activePage=e;const s=this.element.querySelectorAll(".site-header__nav-link");for(const t of s){const i=t.dataset.page;t.classList.toggle("site-header__nav-link--active",i===e)}const a=this.mobileMenuOverlay.querySelectorAll(".mobile-menu__nav-link");for(const t of a){const i=t.dataset.page;t.classList.toggle("mobile-menu__nav-link--active",i===e)}}createHeaderElement(){const e=document.createElement("header");e.className="site-header";const s=document.createElement("div");s.className="site-header__container";const a=document.createElement("a");a.href="#",a.className="site-header__logo",a.innerHTML=`
    ...
    `,a.addEventListener("click",u=>{var d,_;u.preventDefault(),this.setActivePage("home"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"home")});const t=document.createElement("nav");t.className="site-header__nav",t.innerHTML=`
    ...
    `;const i=t.querySelector('[data-page="home"]');i&&i.addEventListener("click",u=>{var d,_;u.preventDefault(),this.setActivePage("home"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"home")});const o=t.querySelector('[data-page="library"]');o&&o.addEventListener("click",u=>{var d,_;u.preventDefault(),this.setActivePage("library"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"library")});const n=document.createElement("button");n.className="btn btn--outline site-header__login-btn",n.textContent="Log In",n.addEventListener("click",()=>{var u,d;this.closeMobileMenu(),(d=(u=this.callbacks).onLoginClick)==null||d.call(u)});const g=document.createElement("button");g.className="btn btn--primary site-header__signup-btn",g.textContent="Sign Up",g.addEventListener("click",()=>{var u,d;this.closeMobileMenu(),(d=(u=this.callbacks).onSignUpClick)==null||d.call(u)});const l=document.createElement("div");l.className="site-header__nav-wrapper",l.append(t,n),this.burgerButton=document.createElement("button"),this.burgerButton.className="site-header__burger-btn",this.burgerButton.setAttribute("aria-label","Toggle menu"),this.burgerButton.innerHTML=`
      <span class="site-header__burger-icon">
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
      </span>
    `;const p=document.createElement("div");p.className="site-header__right-controls",p.append(l,g,this.burgerButton),s.append(a,p),e.append(s),this.mobileMenuOverlay=document.createElement("div"),this.mobileMenuOverlay.className="mobile-menu",this.mobileMenuOverlay.innerHTML=`
    ...
    `;const c=this.mobileMenuOverlay.querySelector(".mobile-menu__logo");c&&c.addEventListener("click",u=>{var d,_;u.preventDefault(),this.closeMobileMenu(),this.setActivePage("home"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"home")});const h=this.mobileMenuOverlay.querySelector('[data-page="home"]');h&&h.addEventListener("click",u=>{var d,_;u.preventDefault(),this.closeMobileMenu(),this.setActivePage("home"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"home")});const f=this.mobileMenuOverlay.querySelector('[data-page="library"]');f&&f.addEventListener("click",u=>{var d,_;u.preventDefault(),this.closeMobileMenu(),this.setActivePage("library"),(_=(d=this.callbacks).onNavigate)==null||_.call(d,"library")});const b=this.mobileMenuOverlay.querySelector(".mobile-menu__close-btn");b&&b.addEventListener("click",()=>{this.closeMobileMenu()});const k=this.mobileMenuOverlay.querySelector(".mobile-menu__login-btn");k&&k.addEventListener("click",()=>{var u,d;this.closeMobileMenu(),(d=(u=this.callbacks).onLoginClick)==null||d.call(u)});const v=this.mobileMenuOverlay.querySelector(".mobile-menu__signup-btn");return v&&v.addEventListener("click",()=>{var u,d;this.closeMobileMenu(),(d=(u=this.callbacks).onSignUpClick)==null||d.call(u)}),e.append(this.mobileMenuOverlay),e}setupEventListeners(){this.burgerButton.addEventListener("click",()=>{this.toggleMobileMenu()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&this.isMenuOpen&&this.closeMobileMenu()})}toggleMobileMenu(){this.isMenuOpen?this.closeMobileMenu():this.openMobileMenu()}openMobileMenu(){this.isMenuOpen=!0,this.burgerButton.classList.add("site-header__burger-btn--open"),this.mobileMenuOverlay.classList.add("mobile-menu--open"),document.body.style.overflow="hidden"}closeMobileMenu(){this.isMenuOpen=!1,this.burgerButton.classList.remove("site-header__burger-btn--open"),this.mobileMenuOverlay.classList.remove("mobile-menu--open"),document.body.style.overflow=""}}const y="data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M3.825%209L9.425%2014.6L8%2016L1.19209e-07%208L8%20-9.53674e-07L9.425%201.4L3.825%207H16V9H3.825Z'%20fill='%23242145'/%3e%3c/svg%3e",A=""+new URL("cat-mail-co-card-B0GMTC_n.jpg",import.meta.url).href,Y=Object.freeze(Object.defineProperty({__proto__:null,default:A},Symbol.toStringTag,{value:"Module"})),j=""+new URL("heartopia-card-DRd_6OVG.jpg",import.meta.url).href,J=Object.freeze(Object.defineProperty({__proto__:null,default:j},Symbol.toStringTag,{value:"Module"})),x=""+new URL("islanders-new-shores-card-DljrohUL.jpg",import.meta.url).href,ee=Object.freeze(Object.defineProperty({__proto__:null,default:x},Symbol.toStringTag,{value:"Module"})),z=""+new URL("palia-card-8xT8yeZQ.jpg",import.meta.url).href,te=Object.freeze(Object.defineProperty({__proto__:null,default:z},Symbol.toStringTag,{value:"Module"})),P=""+new URL("shelve-the-potions-card-DTY_N_zq.jpg",import.meta.url).href,ae=Object.freeze(Object.defineProperty({__proto__:null,default:P},Symbol.toStringTag,{value:"Module"})),T=""+new URL("tailside-cozy-cafe-sim-card-C7B0rePC.jpg",import.meta.url).href,se=Object.freeze(Object.defineProperty({__proto__:null,default:T},Symbol.toStringTag,{value:"Module"})),I=""+new URL("tiny-glade-card-CS2XLEzK.jpg",import.meta.url).href,ie=Object.freeze(Object.defineProperty({__proto__:null,default:I},Symbol.toStringTag,{value:"Module"})),O=""+new URL("vacation-cafe-simulator-card-Bzcyczbo.jpg",import.meta.url).href,re=Object.freeze(Object.defineProperty({__proto__:null,default:O},Symbol.toStringTag,{value:"Module"})),D=""+new URL("winter-burrow-card-KbzzF82b.jpg",import.meta.url).href,oe=Object.freeze(Object.defineProperty({__proto__:null,default:D},Symbol.toStringTag,{value:"Module"}));class ne{constructor(e={}){r(this,"element");r(this,"games",[{slug:"vacation-cafe-simulator",name:"Vacation Cafe Simulator",rating:4.8,likesCount:28750,likesFormatted:"28.7K",cardImage:O},{slug:"winter-burrow",name:"Winter Burrow",rating:4.9,likesCount:32400,likesFormatted:"32.4K",cardImage:D},{slug:"shelve-the-potions",name:"Shelve the Potions!",rating:4.7,likesCount:21300,likesFormatted:"21.3K",cardImage:P},{slug:"heartopia",name:"Heartopia",rating:4.6,likesCount:46800,likesFormatted:"46.8K",cardImage:j},{slug:"palia",name:"Palia",rating:4.9,likesCount:42100,likesFormatted:"42.1K",cardImage:z},{slug:"cat-mail-co",name:"Cat Mail Co.",rating:4.8,likesCount:38900,likesFormatted:"38.9K",cardImage:A},{slug:"tiny-glade",name:"Tiny Glade",rating:4.9,likesCount:51200,likesFormatted:"51.2K",cardImage:I},{slug:"tailside-cozy-cafe-sim",name:"Tailside: Cozy Cafe Sim",rating:4.8,likesCount:18400,likesFormatted:"18.4K",cardImage:T},{slug:"islanders-new-shores",name:"ISLANDERS: New Shores",rating:4.9,likesCount:54200,likesFormatted:"54.2K",cardImage:x}]);r(this,"centerIndex",0);r(this,"autoplayTimer");r(this,"callbacks");r(this,"startX",0);r(this,"isSwiping",!1);this.callbacks=e,this.element=this.createCarouselElement(),this.renderTrack(),this.setupControls(),this.startAutoplay()}getElement(){return this.element}createCarouselElement(){const e=document.createElement("section");return e.className="carousel-section",e.innerHTML=`
    ...
    `,e}getVisibleGames(){const e=this.games.length;return["narrow","normal","wide","normal","narrow"].map((a,t)=>{const i=(this.centerIndex+(t-2)+e*10)%e;return{game:this.games[i],widthType:a}})}renderTrack(){const e=this.element.querySelector(".carousel-section__track");if(!e)return;const s=this.getVisibleGames();e.innerHTML=s.map(t=>`
      <div class="game-card game-card--${t.widthType}" tabindex="0" role="button" aria-label="${t.game.name}" data-slug="${t.game.slug}">
        <img src="${t.game.cardImage}" alt="${t.game.name}" class="game-card__image" />
        <div class="game-card__overlay">
          <div class="game-card__info">
            <h3 class="game-card__title">${t.game.name}</h3>
            <div class="game-card__meta">
              ...
              <span class="game-card__likes">
                ...
                ${t.game.likesFormatted}
              </span>
            </div>
          </div>
        </div>
      </div>
    `).join("");const a=e.querySelectorAll(".game-card");for(const t of a){const i=t.dataset.slug,o=this.games.find(n=>n.slug===i);o&&(t.addEventListener("click",()=>{var n,g;(g=(n=this.callbacks).onGameClick)==null||g.call(n,o)}),t.addEventListener("keydown",n=>{var g,l;(n.key==="Enter"||n.key===" ")&&(n.preventDefault(),(l=(g=this.callbacks).onGameClick)==null||l.call(g,o))}))}this.checkCardWidthThresholds()}checkCardWidthThresholds(){const e=this.element.querySelector(".carousel-section__track");if(!e)return;const s=e.querySelectorAll(".game-card");for(const a of s){const t=a.querySelector(".game-card__overlay");if(t){const i=a.getBoundingClientRect().width;i>0&&i<288?t.style.display="none":i>=288&&(t.style.display="")}}}next(){this.centerIndex=(this.centerIndex+1)%this.games.length,this.renderTrack()}prev(){this.centerIndex=(this.centerIndex-1+this.games.length)%this.games.length,this.renderTrack()}startAutoplay(){this.stopAutoplay(),this.autoplayTimer=setInterval(()=>{this.next()},4e3)}stopAutoplay(){this.autoplayTimer!==void 0&&(clearInterval(this.autoplayTimer),this.autoplayTimer=void 0)}resetAutoplay(){this.startAutoplay()}setupControls(){const e=this.element.querySelector(".carousel-section__control-btn--prev"),s=this.element.querySelector(".carousel-section__control-btn--next");e&&e.addEventListener("click",()=>{this.prev(),this.resetAutoplay()}),s&&s.addEventListener("click",()=>{this.next(),this.resetAutoplay()});const a=this.element.querySelector(".carousel-section__track-wrapper");a&&(a.addEventListener("pointerdown",t=>{this.startX=t.clientX,this.isSwiping=!0,this.stopAutoplay()}),a.addEventListener("pointerup",t=>{if(!this.isSwiping)return;this.isSwiping=!1;const i=t.clientX-this.startX;Math.abs(i)>40?(i<0?this.next():this.prev(),this.resetAutoplay()):this.startAutoplay()}),a.addEventListener("pointercancel",()=>{this.isSwiping=!1,this.startAutoplay()}),a.addEventListener("mouseenter",()=>{this.stopAutoplay()}),a.addEventListener("mouseleave",()=>{this.isSwiping=!1,this.startAutoplay()})),window.addEventListener("resize",()=>{this.checkCardWidthThresholds()})}destroy(){this.stopAutoplay()}}const le=""+new URL("random-image-DsQZrXDY.png",import.meta.url).href,ce="data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M8%202V10M4.66667%205.33333L8%202L11.3333%205.33333M14%2010V12.6667C14%2013.0203%2013.8595%2013.3594%2013.6095%2013.6095C13.3594%2013.8595%2013.0203%2014%2012.6667%2014H3.33333C2.97971%2014%202.64057%2013.8595%202.39052%2013.6095C2.14048%2013.3594%202%2013.0203%202%2012.6667V10'%20stroke='%23242145'%20stroke-width='2'%20stroke-linecap='round'/%3e%3c/svg%3e";class de{constructor(){r(this,"element");this.element=this.createGameDevElement()}getElement(){return this.element}createGameDevElement(){const e=document.createElement("section");return e.className="game-dev-section",e.innerHTML=`
    ...
    `,e}}class ge{constructor(){r(this,"element");this.element=this.createHeroElement()}getElement(){return this.element}createHeroElement(){const e=document.createElement("section");return e.className="hero-section",e.innerHTML=`
      ...
      <div class="leaderboard-section__container">
        ...
        <div class="leaderboard-table-card">
          <table class="leaderboard-table">
            ...
            <tbody>
              ${s.map(a=>`
                <tr class="leaderboard-row leaderboard-row--rank-${a.rank}">
                  <td class="col-rank">
                    <span class="rank-badge rank-badge--${a.rank}">#${a.rank}</span>
                  </td>
                  <td class="col-player">
                    <div class="player-info">
                      <span class="player-avatar" style="background-color: ${he[a.rank]||"#E0EEF6"}">
                        ${pe(a.playerName)}
                      </span>
                      <span class="player-name">${a.playerName}</span>
                    </div>
                  </td>
                  <td class="col-games">${a.gamesPlayed}</td>
                  <td class="col-score">
                    <span class="score-desktop">${E(a.totalScore,!1)}</span>
                    <span class="score-mobile">${E(a.totalScore,!0)}</span>
                  </td>
                  <td class="col-streak">
                    <span class="streak-full">🔥 ${a.streakDays} days</span>
                    <span class="streak-short">🔥 ${a.streakDays}d</span>
                  </td>
                  <td class="col-favorite">
                    <span class="favorite-badge">${a.favoriteGameName}</span>
                  </td>
                </tr>
              `).join("")}
            </tbody>
        ...
        <div class="library-filter-section__controls-row">
          <div class="library-filter-section__chips-wrapper" tabindex="0" role="region" aria-label="Categories">
            <div class="library-filter-section__chips-row" role="tablist">
              ${s.map(t=>`
                <button
                  type="button"
                  class="library-filter-section__chip ${t.slug===this.activeCategory?"library-filter-section__chip--active":""}"
                  data-category="${t.slug}"
                  role="tab"
                  aria-selected="${t.slug===this.activeCategory}"
                >
                  ${t.label}
                </button>
              `).join("")}
          ...
          <div class="library-filter-section__sort-container">
            ...
            <ul class="library-filter-section__sort-dropdown" role="listbox">
              ${a.map(t=>`
                <li class="library-filter-section__sort-item ${t===this.activeSort?"library-filter-section__sort-item--active":""}" role="option" data-sort="${t}">
                  ${t}
                </li>
              `).join("")}
            </ul>
    ...
    `,e}renderCards(){const e=this.element.querySelector(".library-grid-section__grid");if(!e)return;const s=(this.currentPage-1)*this.itemsPerPage,a=this.games.slice(s,s+this.itemsPerPage);e.innerHTML=a.map(i=>{const o=this.getGameImageUrl(i.slug),n=this.formatLikes(i.likesCount);return`
        <article class="library-card" data-slug="${i.slug}">
          <div class="library-card__thumb-wrap">
            <img src="${o}" alt="${i.name}" class="library-card__image" loading="lazy" />
          </div>
          <div class="library-card__body">
            <div class="library-card__top-row">
              <div class="library-card__title-group">
                <h2 class="library-card__title">${i.name}</h2>
                <span class="library-card__category">${i.category}</span>
              </div>
              <span class="library-card__price ${i.price==="Free"?"":"library-card__price--paid"}">${i.price}</span>
            </div>

            <p class="library-card__description">${i.shortDescription}</p>
            ...
            <div class="library-card__bottom-row">
              ...
              <button type="button" class="library-card__details-btn" data-slug="${i.slug}">Details</button>
            </div>
          </div>
        </article>
      `}).join("");const t=e.querySelectorAll(".library-card__details-btn");for(const i of t)i.addEventListener("click",()=>{var g,l;const o=i.dataset.slug,n=this.games.find(p=>p.slug===o);n&&((l=(g=this.callbacks).onGameDetailsClick)==null||l.call(g,n))})}setupInteractivity(){const e=this.element.querySelectorAll(".library-filter-section__chip");for(const c of e)c.addEventListener("click",()=>{const h=c.dataset.category;if(h){this.activeCategory=h;for(const f of e){const b=f.dataset.category===h;f.classList.toggle("library-filter-section__chip--active",b),f.setAttribute("aria-selected",b.toString())}}});const s=this.element.querySelector(".library-filter-section__chips-wrapper");if(s){let c=!1,h=0,f=0;s.addEventListener("mousedown",b=>{c=!0,h=b.pageX-s.offsetLeft,f=s.scrollLeft}),s.addEventListener("mouseleave",()=>{c=!1}),s.addEventListener("mouseup",()=>{c=!1}),s.addEventListener("mousemove",b=>{if(!c)return;b.preventDefault();const v=(b.pageX-s.offsetLeft-h)*1.5;s.scrollLeft=f-v})}const a=this.element.querySelector(".library-filter-section__sort-btn"),t=this.element.querySelector(".library-filter-section__sort-dropdown"),i=this.element.querySelectorAll(".library-filter-section__sort-item"),o=this.element.querySelector(".sort-current-label");if(a&&t){a.addEventListener("click",c=>{c.stopPropagation(),this.isSortOpen=!this.isSortOpen,a.classList.toggle("library-filter-section__sort-btn--open",this.isSortOpen),t.classList.toggle("library-filter-section__sort-dropdown--open",this.isSortOpen),a.setAttribute("aria-expanded",this.isSortOpen.toString())});for(const c of i)c.addEventListener("click",()=>{const h=c.dataset.sort;if(h){this.activeSort=h,o&&(o.textContent=h);for(const f of i)f.classList.toggle("library-filter-section__sort-item--active",f.dataset.sort===h);this.isSortOpen=!1,a.classList.remove("library-filter-section__sort-btn--open"),t.classList.remove("library-filter-section__sort-dropdown--open"),a.setAttribute("aria-expanded","false")}});document.addEventListener("click",c=>{this.element.contains(c.target)||(this.isSortOpen=!1,a.classList.remove("library-filter-section__sort-btn--open"),t.classList.remove("library-filter-section__sort-dropdown--open"),a.setAttribute("aria-expanded","false"))})}const n=this.element.querySelector(".library-pagination__arrow-btn--prev"),g=this.element.querySelector(".library-pagination__arrow-btn--next"),l=this.element.querySelectorAll(".library-pagination__page-btn"),p=c=>{this.currentPage=c,n&&(n.disabled=this.currentPage===1),g&&(g.disabled=this.currentPage===this.totalPages);for(const h of l){const f=Number(h.dataset.page);h.classList.toggle("library-pagination__page-btn--active",f===c)}this.renderCards()};for(const c of l)c.addEventListener("click",()=>{const h=Number(c.dataset.page);h&&p(h)});n&&n.addEventListener("click",()=>{this.currentPage>1&&p(this.currentPage-1)}),g&&g.addEventListener("click",()=>{this.currentPage<this.totalPages&&p(this.currentPage+1)})}}class Ye{constructor(e){r(this,"routes",[]);r(this,"rootElement");r(this,"currentPath","");this.rootElement=e,window.addEventListener("popstate",()=>{this.handleRoute()}),window.addEventListener("hashchange",()=>{this.handleRoute()})}addRoute(e){this.routes.push(e)}navigate(e){window.location.hash=e.startsWith("#")?e:`#${e}`}getCurrentRoute(){return window.location.hash.replace(/^#\/?/,"")==="library"||window.location.pathname.replaceAll(/^\/|\/$/g,"").endsWith("library")?"library":"home"}handleRoute(){const s=this.getCurrentRoute()==="library"?"/library":"/";if(this.currentPath===s&&this.rootElement.children.length>0)return;this.currentPath=s;const a=this.routes.find(t=>t.path===s);if(this.rootElement.innerHTML="",a)this.rootElement.append(a.render());else{const t=this.routes.find(i=>i.path==="/");t&&this.rootElement.append(t.render())}}}function Je(){var g;const m=document.querySelector("#app");if(!m)return;const e=new Ye(m),s=new R,a=new q,t=new Q({activePage:"home",onLoginClick:()=>{s.open("login")},onSignUpClick:()=>{s.open("register")},onNavigate:l=>{e.navigate(l==="library"?"/library":"/")}}),i=new X,n=i.getElement().querySelectorAll(".site-footer__link");for(const l of n){const p=(g=l.textContent)==null?void 0:g.trim().toLowerCase();p==="home"?l.addEventListener("click",c=>{c.preventDefault(),t.setActivePage("home"),e.navigate("/")}):p==="library"&&l.addEventListener("click",c=>{c.preventDefault(),t.setActivePage("library"),e.navigate("/library")})}e.addRoute({path:"/",render:()=>{t.setActivePage("home");const l=document.createElement("div");l.className="app-container app-container--home",l.append(t.getElement());const p=document.createElement("main");p.className="main-content";const c=new ge,h=new ne({onGameClick:()=>{a.open()}}),f=new _e,b=new de;return p.append(c.getElement(),h.getElement(),f.getElement(),b.getElement()),l.append(p,i.getElement()),l}}),e.addRoute({path:"/library",render:()=>{t.setActivePage("library");const l=document.createElement("div");l.className="app-container app-container--library",l.append(t.getElement());const p=document.createElement("main");p.className="main-content";const c=new Qe({onGameDetailsClick:()=>{a.open()}});return p.append(c.getElement()),l.append(p,i.getElement()),l}}),e.handleRoute()}document.addEventListener("DOMContentLoaded",()=>{Je()});
