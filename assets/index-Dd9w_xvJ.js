var D=Object.defineProperty;var B=(m,e,s)=>e in m?D(m,e,{enumerable:!0,configurable:!0,writable:!0,value:s}):m[e]=s;var o=(m,e,s)=>B(m,typeof e!="symbol"?e+"":e,s);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))t(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const i of r.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function s(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function t(a){if(a.ep)return;a.ep=!0;const r=s(a);fetch(a.href,r)}})();class N{constructor(){o(this,"element");o(this,"backdrop");o(this,"dialogCard");o(this,"currentMode","login");o(this,"isOpen",!1);o(this,"handleKeyDown",e=>{e.key==="Escape"&&this.close()});this.backdrop=document.createElement("div"),this.backdrop.className="auth-backdrop",this.dialogCard=document.createElement("div"),this.dialogCard.className="auth-dialog",this.backdrop.append(this.dialogCard),this.element=this.backdrop,this.renderContent(),this.attachEvents()}getElement(){return this.element}open(e="login"){this.currentMode=e,this.renderContent(),document.body.append(this.element),this.isOpen=!0,requestAnimationFrame(()=>{this.backdrop.classList.add("auth-backdrop--active"),this.dialogCard.classList.add("auth-dialog--active")}),document.addEventListener("keydown",this.handleKeyDown)}close(){this.isOpen&&(this.backdrop.classList.remove("auth-backdrop--active"),this.dialogCard.classList.remove("auth-dialog--active"),setTimeout(()=>{this.element.parentNode&&this.element.remove(),this.isOpen=!1},250),document.removeEventListener("keydown",this.handleKeyDown))}attachEvents(){this.backdrop.addEventListener("click",e=>{e.target===this.backdrop&&this.close()})}switchMode(e){if(this.currentMode===e)return;this.currentMode=e;const s=this.dialogCard.querySelector(".auth-dialog__form-container");s?(s.classList.add("auth-dialog__form-container--transitioning"),setTimeout(()=>{this.renderContent();const t=this.dialogCard.querySelector(".auth-dialog__form-container");t&&t.classList.remove("auth-dialog__form-container--transitioning")},150)):this.renderContent()}renderContent(){const e=this.currentMode==="login";this.dialogCard.innerHTML=`
    ...
    `;const s=this.dialogCard.querySelectorAll(".auth-dialog__switcher-btn");for(const i of s)i.addEventListener("click",()=>{const n=i.dataset.mode;this.switchMode(n)});const t=this.dialogCard.querySelectorAll(".auth-switch-link");for(const i of t)i.addEventListener("click",n=>{n.preventDefault();const g=i.dataset.mode;this.switchMode(g)});const a=this.dialogCard.querySelectorAll(".auth-field__toggle-pwd");for(const i of a)i.addEventListener("click",()=>{var g;const n=(g=i.parentElement)==null?void 0:g.querySelector("input");if(n){const d=n.type==="password";n.type=d?"text":"password",i.innerHTML=d?this.getEyeOffIconSvg():this.getEyeIconSvg()}});const r=this.dialogCard.querySelector("form");r&&r.addEventListener("submit",i=>{i.preventDefault()})}renderLoginForm(){return`
            ...
            <section class="game-details-dialog__records-section">
              ...
              <table class="game-details-dialog__records-table">
                ...
                <tbody>
                  ${s.topRecords.map(a=>`
                    <tr>
                      <td class="record-rank">${a.position}</td>
                      <td class="record-player">${a.playerName}</td>
                      <td class="record-score">${a.score.toLocaleString()}</td>
                    </tr>
                  `).join("")}
                </tbody>
              </table>
                ...
                ${t.map(a=>`
                  <article class="game-details-dialog__comment-item">
                    <div class="game-details-dialog__comment-header">
                      ...
                      <time class="game-details-dialog__comment-date">${new Date(a.createdAt).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</time>
                    </div>
    ...
    `,e}setupListeners(){this.backdropElement.addEventListener("click",a=>{a.target===this.backdropElement&&this.close()});const e=this.backdropElement.querySelector(".game-details-dialog__hero-close-btn");e&&e.addEventListener("click",()=>{this.close()}),document.addEventListener("keydown",a=>{a.key==="Escape"&&this.backdropElement.classList.contains("game-details-backdrop--active")&&this.close()}),this.favoriteBtnElement=this.backdropElement.querySelector(".game-details-dialog__favorite-btn")??void 0,this.favoriteBtnElement&&this.favoriteBtnElement.addEventListener("click",()=>{var r,i,n;this.isFavorite=!this.isFavorite,(r=this.favoriteBtnElement)==null||r.classList.toggle("game-details-dialog__favorite-btn--active",this.isFavorite);const a=(i=this.favoriteBtnElement)==null?void 0:i.querySelector(".fav-btn-text");a&&(a.textContent=this.isFavorite?"Remove from Favorites":"Add to Favorites"),(n=this.favoriteBtnElement)==null||n.setAttribute("aria-label",this.isFavorite?"Remove from Favorites":"Add to Favorites")}),this.textareaElement=this.backdropElement.querySelector(".game-details-dialog__textarea")??void 0;const s=this.backdropElement.querySelector(".game-details-dialog__submit-comment-btn");this.textareaElement&&this.textareaElement.addEventListener("input",()=>{if(!this.textareaElement)return;this.textareaElement.style.height="auto";const a=Math.min(this.textareaElement.scrollHeight,88);this.textareaElement.style.height=`${a}px`,s&&(s.disabled=this.textareaElement.value.trim().length===0)}),s&&s.addEventListener("click",()=>{if(!this.textareaElement||this.textareaElement.value.trim().length===0)return;const a=this.textareaElement.value.trim(),r=this.backdropElement.querySelector(".game-details-dialog__comments-list");if(r){const i=document.createElement("article");i.className="game-details-dialog__comment-item";const n=`comment-${Date.now()}`;this.commentLikes.set(n,{count:0,active:!1}),i.innerHTML=`
            <div class="game-details-dialog__comment-header">
              <span class="game-details-dialog__comment-author">You</span>
              <time class="game-details-dialog__comment-date">Just now</time>
            </div>
            <p class="game-details-dialog__comment-text">${a}</p>
            <div class="game-details-dialog__comment-footer">
              <button
                type="button"
                class="game-details-dialog__comment-like-btn"
    ...
    `,e}}class Y{constructor(e={}){o(this,"element");o(this,"isMenuOpen",!1);o(this,"burgerButton");o(this,"mobileMenuOverlay");o(this,"callbacks");o(this,"activePage");this.callbacks=e,this.activePage=e.activePage||"home",this.element=this.createHeaderElement(),this.setupEventListeners()}getElement(){return this.element}setActivePage(e){this.activePage=e;const s=this.element.querySelectorAll(".site-header__nav-link");for(const a of s){const r=a.dataset.page;a.classList.toggle("site-header__nav-link--active",r===e)}const t=this.mobileMenuOverlay.querySelectorAll(".mobile-menu__nav-link");for(const a of t){const r=a.dataset.page;a.classList.toggle("mobile-menu__nav-link--active",r===e)}}createHeaderElement(){const e=document.createElement("header");e.className="site-header";const s=document.createElement("div");s.className="site-header__container";const t=document.createElement("a");t.href="#",t.className="site-header__logo",t.innerHTML=`
    ...
    `,t.addEventListener("click",h=>{var c,f;h.preventDefault(),this.setActivePage("home"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"home")});const a=document.createElement("nav");a.className="site-header__nav",a.innerHTML=`
    ...
    `;const r=a.querySelector('[data-page="home"]');r&&r.addEventListener("click",h=>{var c,f;h.preventDefault(),this.setActivePage("home"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"home")});const i=a.querySelector('[data-page="library"]');i&&i.addEventListener("click",h=>{var c,f;h.preventDefault(),this.setActivePage("library"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"library")});const n=document.createElement("button");n.className="btn btn--outline site-header__login-btn",n.textContent="Log In",n.addEventListener("click",()=>{var h,c;this.closeMobileMenu(),(c=(h=this.callbacks).onLoginClick)==null||c.call(h)});const g=document.createElement("button");g.className="btn btn--primary site-header__signup-btn",g.textContent="Sign Up",g.addEventListener("click",()=>{var h,c;this.closeMobileMenu(),(c=(h=this.callbacks).onSignUpClick)==null||c.call(h)});const d=document.createElement("div");d.className="site-header__nav-wrapper",d.append(a,n),this.burgerButton=document.createElement("button"),this.burgerButton.className="site-header__burger-btn",this.burgerButton.setAttribute("aria-label","Toggle menu"),this.burgerButton.innerHTML=`
      <span class="site-header__burger-icon">
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
        <span class="site-header__burger-line"></span>
      </span>
    `;const p=document.createElement("div");p.className="site-header__right-controls",p.append(d,g,this.burgerButton),s.append(t,p),e.append(s),this.mobileMenuOverlay=document.createElement("div"),this.mobileMenuOverlay.className="mobile-menu",this.mobileMenuOverlay.innerHTML=`
    ...
    `;const l=this.mobileMenuOverlay.querySelector(".mobile-menu__logo");l&&l.addEventListener("click",h=>{var c,f;h.preventDefault(),this.closeMobileMenu(),this.setActivePage("home"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"home")});const u=this.mobileMenuOverlay.querySelector('[data-page="home"]');u&&u.addEventListener("click",h=>{var c,f;h.preventDefault(),this.closeMobileMenu(),this.setActivePage("home"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"home")});const _=this.mobileMenuOverlay.querySelector('[data-page="library"]');_&&_.addEventListener("click",h=>{var c,f;h.preventDefault(),this.closeMobileMenu(),this.setActivePage("library"),(f=(c=this.callbacks).onNavigate)==null||f.call(c,"library")});const b=this.mobileMenuOverlay.querySelector(".mobile-menu__close-btn");b&&b.addEventListener("click",()=>{this.closeMobileMenu()});const v=this.mobileMenuOverlay.querySelector(".mobile-menu__login-btn");v&&v.addEventListener("click",()=>{var h,c;this.closeMobileMenu(),(c=(h=this.callbacks).onLoginClick)==null||c.call(h)});const y=this.mobileMenuOverlay.querySelector(".mobile-menu__signup-btn");return y&&y.addEventListener("click",()=>{var h,c;this.closeMobileMenu(),(c=(h=this.callbacks).onSignUpClick)==null||c.call(h)}),e.append(this.mobileMenuOverlay),e}setupEventListeners(){this.burgerButton.addEventListener("click",()=>{this.toggleMobileMenu()}),document.addEventListener("keydown",e=>{e.key==="Escape"&&this.isMenuOpen&&this.closeMobileMenu()})}toggleMobileMenu(){this.isMenuOpen?this.closeMobileMenu():this.openMobileMenu()}openMobileMenu(){this.isMenuOpen=!0,this.burgerButton.classList.add("site-header__burger-btn--open"),this.mobileMenuOverlay.classList.add("mobile-menu--open"),document.body.style.overflow="hidden"}closeMobileMenu(){this.isMenuOpen=!1,this.burgerButton.classList.remove("site-header__burger-btn--open"),this.mobileMenuOverlay.classList.remove("mobile-menu--open"),document.body.style.overflow=""}}const k="data:image/svg+xml,%3csvg%20width='16'%20height='16'%20viewBox='0%200%2016%2016'%20fill='none'%20xmlns='http://www.w3.org/2000/svg'%3e%3cpath%20d='M3.825%209L9.425%2014.6L8%2016L1.19209e-07%208L8%20-9.53674e-07L9.425%201.4L3.825%207H16V9H3.825Z'%20fill='%23242145'/%3e%3c/svg%3e",M=""+new URL("cat-mail-co-card-B0GMTC_n.jpg",import.meta.url).href,Q=Object.freeze(Object.defineProperty({__proto__:null,default:M},Symbol.toStringTag,{value:"Module"})),x=""+new URL("heartopia-card-DRd_6OVG.jpg",import.meta.url).href,J=Object.freeze(Object.defineProperty({__proto__:null,default:x},Symbol.toStringTag,{value:"Module"})),j=""+new URL("islanders-new-shores-card-DljrohUL.jpg",import.meta.url).href,ee=Object.freeze(Object.defineProperty({__proto__:null,default:j},Symbol.toStringTag,{value:"Module"})),z=""+new URL("palia-card-8xT8yeZQ.jpg",import.meta.url).href,te=Object.freeze(Object.defineProperty({__proto__:null,default:z},Symbol.toStringTag,{value:"Module"})),P=""+new URL("shelve-the-potions-card-DTY_N_zq.jpg",import.meta.url).href,ae=Object.freeze(Object.defineProperty({__proto__:null,default:P},Symbol.toStringTag,{value:"Module"})),I=""+new URL("tailside-cozy-cafe-sim-card-C7B0rePC.jpg",import.meta.url).href,se=Object.freeze(Object.defineProperty({__proto__:null,default:I},Symbol.toStringTag,{value:"Module"})),T=""+new URL("tiny-glade-card-CS2XLEzK.jpg",import.meta.url).href,ie=Object.freeze(Object.defineProperty({__proto__:null,default:T},Symbol.toStringTag,{value:"Module"})),F=""+new URL("vacation-cafe-simulator-card-Bzcyczbo.jpg",import.meta.url).href,re=Object.freeze(Object.defineProperty({__proto__:null,default:F},Symbol.toStringTag,{value:"Module"})),O=""+new URL("winter-burrow-card-KbzzF82b.jpg",import.meta.url).href,oe=Object.freeze(Object.defineProperty({__proto__:null,default:O},Symbol.toStringTag,{value:"Module"}));class ne{constructor(e={}){o(this,"element");o(this,"games",[{slug:"vacation-cafe-simulator",name:"Vacation Cafe Simulator",rating:4.8,likesCount:28750,likesFormatted:"28.7K",cardImage:F},{slug:"winter-burrow",name:"Winter Burrow",rating:4.9,likesCount:32400,likesFormatted:"32.4K",cardImage:O},{slug:"shelve-the-potions",name:"Shelve the Potions!",rating:4.7,likesCount:21300,likesFormatted:"21.3K",cardImage:P},{slug:"heartopia",name:"Heartopia",rating:4.6,likesCount:46800,likesFormatted:"46.8K",cardImage:x},{slug:"palia",name:"Palia",rating:4.9,likesCount:42100,likesFormatted:"42.1K",cardImage:z},{slug:"cat-mail-co",name:"Cat Mail Co.",rating:4.8,likesCount:38900,likesFormatted:"38.9K",cardImage:M},{slug:"tiny-glade",name:"Tiny Glade",rating:4.9,likesCount:51200,likesFormatted:"51.2K",cardImage:T},{slug:"tailside-cozy-cafe-sim",name:"Tailside: Cozy Cafe Sim",rating:4.8,likesCount:18400,likesFormatted:"18.4K",cardImage:I},{slug:"islanders-new-shores",name:"ISLANDERS: New Shores",rating:4.9,likesCount:54200,likesFormatted:"54.2K",cardImage:j}]);o(this,"centerIndex",0);o(this,"autoplayTimer");o(this,"callbacks");o(this,"startX",0);o(this,"isSwiping",!1);o(this,"hasSwiped",!1);o(this,"isAnimating",!1);this.callbacks=e,this.element=this.createCarouselElement(),this.renderTrack(),this.setupControls(),this.startAutoplay()}getElement(){return this.element}createCarouselElement(){const e=document.createElement("section");return e.className="carousel-section",e.innerHTML=`
    ...
    `,e}getVisibleGames(){const e=this.games.length;return["narrow","normal","wide","normal","narrow"].map((t,a)=>{const r=(this.centerIndex+(a-2)+e*10)%e;return{game:this.games[r],widthType:t}})}createCardElement(e,s){const t=document.createElement("div");return t.className=`game-card game-card--${s}`,t.tabIndex=0,t.setAttribute("role","button"),t.setAttribute("aria-label",e.name),t.dataset.slug=e.slug,t.innerHTML=`
    ...
    `,e}getFilteredAndSortedGames(){let e=[...this.games];switch(this.activeCategory!=="all"&&(e=e.filter(s=>s.category.toLowerCase()===this.activeCategory.toLowerCase())),this.activeSort){case"Rating ↑":{e.sort((s,t)=>s.rating-t.rating);break}case"Rating ↓":{e.sort((s,t)=>t.rating-s.rating);break}case"Name A→Z":{e.sort((s,t)=>s.name.localeCompare(t.name));break}case"Name Z→A":{e.sort((s,t)=>t.name.localeCompare(s.name));break}}return e}renderCards(){const e=this.element.querySelector(".library-grid-section__grid");if(!e)return;const s=this.getFilteredAndSortedGames();this.totalPages=Math.max(1,Math.ceil(s.length/this.itemsPerPage));const t=(this.currentPage-1)*this.itemsPerPage,a=s.slice(t,t+this.itemsPerPage);e.innerHTML=a.map(i=>{const n=this.getGameImageUrl(i.slug),g=this.formatLikes(i.likesCount);return`
      ...
      `}).join("");const r=e.querySelectorAll(".library-card__details-btn");for(const i of r)i.addEventListener("click",()=>{var d,p;const n=i.dataset.slug,g=this.games.find(l=>l.slug===n);g&&((p=(d=this.callbacks).onGameDetailsClick)==null||p.call(d,g))})}setupInteractivity(){const e=this.element.querySelector(".library-pagination__arrow-btn--prev"),s=this.element.querySelector(".library-pagination__arrow-btn--next"),t=this.element.querySelectorAll(".library-pagination__page-btn"),a=l=>{this.currentPage=Math.min(l,this.totalPages),e&&(e.disabled=this.currentPage<=1),s&&(s.disabled=this.currentPage>=this.totalPages);for(const u of t){const _=Number(u.dataset.page);u.classList.toggle("library-pagination__page-btn--active",_===this.currentPage),u.style.display=_>this.totalPages?"none":""}this.renderCards()},r=this.element.querySelectorAll(".library-filter-section__chip");for(const l of r)l.addEventListener("click",()=>{const u=l.dataset.category;if(u){this.activeCategory=u;for(const _ of r){const b=_.dataset.category===u;_.classList.toggle("library-filter-section__chip--active",b),_.setAttribute("aria-selected",b.toString())}a(1)}});const i=this.element.querySelector(".library-filter-section__chips-wrapper");if(i){let l=!1,u=0,_=0;i.addEventListener("mousedown",b=>{l=!0,u=b.pageX-i.offsetLeft,_=i.scrollLeft}),i.addEventListener("mouseleave",()=>{l=!1}),i.addEventListener("mouseup",()=>{l=!1}),i.addEventListener("mousemove",b=>{if(!l)return;b.preventDefault();const y=(b.pageX-i.offsetLeft-u)*1.5;i.scrollLeft=_-y})}const n=this.element.querySelector(".library-filter-section__sort-btn"),g=this.element.querySelector(".library-filter-section__sort-dropdown"),d=this.element.querySelectorAll(".library-filter-section__sort-item"),p=this.element.querySelector(".sort-current-label");if(n&&g){n.addEventListener("click",l=>{l.stopPropagation(),this.isSortOpen=!this.isSortOpen,n.classList.toggle("library-filter-section__sort-btn--open",this.isSortOpen),g.classList.toggle("library-filter-section__sort-dropdown--open",this.isSortOpen),n.setAttribute("aria-expanded",this.isSortOpen.toString())});for(const l of d)l.addEventListener("click",()=>{const u=l.dataset.sort;if(u){this.activeSort=u,p&&(p.textContent=u);for(const _ of d){const b=_.dataset.sort===u;_.classList.toggle("library-filter-section__sort-item--active",b);const v=_.querySelector(".library-filter-section__sort-check");v&&(v.textContent=b?"✔":"")}this.isSortOpen=!1,n.classList.remove("library-filter-section__sort-btn--open"),g.classList.remove("library-filter-section__sort-dropdown--open"),n.setAttribute("aria-expanded","false"),a(1)}});document.addEventListener("click",l=>{this.element.contains(l.target)||(this.isSortOpen=!1,n.classList.remove("library-filter-section__sort-btn--open"),g.classList.remove("library-filter-section__sort-dropdown--open"),n.setAttribute("aria-expanded","false"))})}for(const l of t)l.addEventListener("click",()=>{const u=Number(l.dataset.page);u&&a(u)});e&&e.addEventListener("click",()=>{this.currentPage>1&&a(this.currentPage-1)}),s&&s.addEventListener("click",()=>{this.currentPage<this.totalPages&&a(this.currentPage+1)})}}class Qe{constructor(e){o(this,"routes",[]);o(this,"rootElement");o(this,"currentPath","");this.rootElement=e,window.addEventListener("popstate",()=>{this.handleRoute()}),window.addEventListener("hashchange",()=>{this.handleRoute()})}addRoute(e){this.routes.push(e)}navigate(e){window.location.hash=e.startsWith("#")?e:`#${e}`}getCurrentRoute(){return window.location.hash.replace(/^#\/?/,"")==="library"||window.location.pathname.replaceAll(/^\/|\/$/g,"").endsWith("library")?"library":"home"}handleRoute(){const s=this.getCurrentRoute()==="library"?"/library":"/";if(this.currentPath===s&&this.rootElement.children.length>0)return;this.currentPath=s;const t=this.routes.find(a=>a.path===s);if(this.rootElement.innerHTML="",t)this.rootElement.append(t.render());else{const a=this.routes.find(r=>r.path==="/");a&&this.rootElement.append(a.render())}}}function Je(){var g;const m=document.querySelector("#app");if(!m)return;const e=new Qe(m),s=new N,t=new G,a=new Y({activePage:"home",onLoginClick:()=>{s.open("login")},onSignUpClick:()=>{s.open("register")},onNavigate:d=>{e.navigate(d==="library"?"/library":"/")}}),r=new X,n=r.getElement().querySelectorAll(".site-footer__link");for(const d of n){const p=(g=d.textContent)==null?void 0:g.trim().toLowerCase();p==="home"?d.addEventListener("click",l=>{l.preventDefault(),a.setActivePage("home"),e.navigate("/")}):p==="library"&&d.addEventListener("click",l=>{l.preventDefault(),a.setActivePage("library"),e.navigate("/library")})}e.addRoute({path:"/",render:()=>{a.setActivePage("home");const d=document.createElement("div");d.className="app-container app-container--home",d.append(a.getElement());const p=document.createElement("main");p.className="main-content";const l=new me,u=new ne({onGameClick:()=>{t.open()}}),_=new _e,b=new de;return p.append(l.getElement(),u.getElement(),_.getElement(),b.getElement()),d.append(p,r.getElement()),d}}),e.addRoute({path:"/library",render:()=>{a.setActivePage("library");const d=document.createElement("div");d.className="app-container app-container--library",d.append(a.getElement());const p=document.createElement("main");p.className="main-content";const l=new Ye({onGameDetailsClick:()=>{t.open()}});return p.append(l.getElement()),d.append(p,r.getElement()),d}}),e.handleRoute()}document.addEventListener("DOMContentLoaded",()=>{Je()});
